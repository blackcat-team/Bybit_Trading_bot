...
* **Фиксированный риск:** Рассчитывает объем позиции (`qty`) по ценовому риску от входа до SL на заданную сумму (например, $50). Комиссии и проскальзывание не входят, поэтому реализованный PnL может отличаться.
...
* **📉 Ступенчатый Безубыток:**
    ...
    * Прибыль ≥ **2R**: Переводит SL в защитную зону (Вход + 0.05R).
...
* **Отчетность:** Ручной отчёт `/report` со статистикой PnL/Winrate за месяц; `/report 01.2026` выгружает месяц в Excel (`.xlsx`). Плюс автоматические дневной и недельный отчёты.
...
Файл должен быть сохранён в **UTF-8** (без BOM), иначе бот не запустится.
...
# reject = отклонить сделку (по умолчанию)
# queue  = сохранить в очередь (автоисполнения нет — только хранение)
HEAT_ACTION=reject
...
| `/status` | Снимок состояния: торговля, дневной PnL, позиции, тепло, источники, последний алерт. |
| `/risk 50` | Показать или задать глобальный ценовой риск сделки (ENTRY→SL); без аргумента показывает текущий. |
| `/pos` | Открытые позиции и кнопки управления защитой (SL/TP). |
| `/orders` | Ордера на вход и кнопки их отмены. |
| `/report` | Сделки за текущий месяц; `/report 01.2026` — выгрузка месяца в Excel (`.xlsx`). |
| `/note BTC Текст` | Записать заметку к инструменту в торговый дневник. |
| `/timeline BTCUSDT` | События инструмента из локального журнала (только чтение, без запросов к Bybit). |
| `/health` | Счётчики обработки команд и статус OK / DEGRADED. |
| `/info` | Справка по командам и синтаксису сигналов. |
| `/price BTC` | Текущая цена инструмента на Bybit Linear (только чтение). |

Все команды доступны только владельцу (`ALLOWED_TELEGRAM_ID`). Ответы бота — на русском.

---

## 📊 Отчётность

- `/report` — ручная интерактивная команда только для владельца. Без аргумента отвечает текстовой сводкой закрытых сделок за текущий месяц (показываются последние 15). С аргументом месяца, например `/report 01.2026`, формирует нативную книгу Excel (`.xlsx`) за этот месяц.
- Автоматические отчёты работают в фоне: ежедневный отчёт по балансу и еженедельный отчёт по источникам сигналов.
- По умолчанию автоматические отчёты уходят владельцу (`ALLOWED_TELEGRAM_ID`). Их можно направить в один произвольный чат или топик форума:

```env
# owner (по умолчанию) = личный чат ALLOWED_TELEGRAM_ID
# custom               = автоматические DAILY/WEEKLY отчёты в один произвольный адрес
TELEGRAM_REPORT_DESTINATION=owner
TELEGRAM_REPORT_CHAT_ID=        # обязателен при TELEGRAM_REPORT_DESTINATION=custom
TELEGRAM_REPORT_THREAD_ID=      # необязательный ID топика форума
```

Произвольный адрес — только исходящий: он лишь определяет, куда доставляются автоматические DAILY/WEEKLY отчёты, и не даёт никому в этом чате прав на команды, колбэки, сигналы или торговлю — отдельного управляющего контура не создаётся. Авторизация команд остаётся прежней проверкой идентификатора владельца (`ALLOWED_TELEGRAM_ID`) и не зависит от типа чата: владелец может вызывать команды из любого чата, где бот их получает. Ручная команда `/report` не затрагивается и использует ту же обычную авторизацию владельца. В режиме `custom` адрес один — дублирующей копии владельцу нет.

---

## 🔢 Порядок включения фич

Рекомендуемая последовательность — от минимального риска к полной автоматике:

| Шаг | Переменная | Значение | Что даёт |
| :-- | :--------- | :------- | :------- |
| 1 | `REQUIRE_MARKET_CONFIRM=1` | `1` | Превью сделки перед исполнением; проще всего откатить |
| 2 | `MAX_TOTAL_HEAT_USDT` | напр. `200` | Ограничение суммарного открытого риска; `HEAT_ACTION=reject` |
| 3 | `CONFLICT_POLICY_SAME_DIR=ignore` | (уже по умолчанию) | Блокирует дублирующиеся сигналы в том же направлении |
| 4 | `QUARANTINE_LOSS_STREAK` | напр. `3` | Автоматически отключает плохо работающий источник после N потерь подряд |

> **Примечание по очереди:** `HEAT_ACTION=queue` сохраняет сигнал, но **не исполняет его автоматически** — требуется отдельная реализация воркера. Для продакшна используйте `reject`.
